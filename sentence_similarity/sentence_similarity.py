...
def sentence_similarity(
    ...
    *,
    ...
    one_hot_tensor = sparse.stack(one_hot_sentences)
...
def _one_hot_sentence(
    ...
    # # This avoids getting high similarity scores because the same word appears multiple
    ...
    # Since words can occur multiple times in a sentence, repeated words need to be discounted
    # so comparing a sentence with itself doesn't increase above 1 due to repeated words.
    counter = Counter(sentence)
    data = [1 / np.sqrt(counter[word]) for word in sentence]

    # index is the location of the word in the sentence, needed to build the sparse matrix
    index = list(range(len(sentence)))

    return sparse.COO(
        [sentence, index],
        data,
        shape=(vocab_length, max_sentence_length),
    )
...
def _einsum(
    tensor: sparse.COO | sparse.GCXS, weight_matrix: np.ndarray
) -> np.ndarray:
    ...
    # sparse version
    # weight_matrix = sparse.COO.from_numpy(weight_matrix)

    similarity = sparse.einsum(
        "mij, nik, jk -> mn", tensor, tensor, weight_matrix, optimize="optimal"
    )
    return similarity.todense()
    # scaling down columnwise by diagonal, this should result in
    # scoring in a column being relative to the sentence itself
    return sparse.einsum(
        "ij, j -> ij", similarity, 1 / sparse.diagonal(similarity)
    )
