...
* Testing merge_vocabs()
from src.translator import merge_vocabs, _restack_vocab

@composite
def restack_vocab_strategy(draw) -> dict[str, int]:
    """Generates a vocab where multiple tokens point to the same integer"""
    # cheating at my own rule, using a strategy in a different strategy, but otherwise this is really annoying
    # generating a vocab with repeating values using the synonym vocab strategy
    _, sentences, synonyms, tokenizer = draw(synonym_vocab_strategy())
    vocab = create_synonym_vocab(sentences, synonyms, tokenizer) # look I tested this alright?!
    return vocab


@given(vocab=restack_vocab_strategy())
def test_restack_vocab(vocab):
    restacked = _restack_vocab(vocab)
    for token in vocab.keys():
        index = vocab[token]
        assert token in restacked[index]

@composite
def merge_vocabs_strategy(draw) -> tuple[set[str], dict[str, int], dict[str, int], list[tuple]]:
    """Generate a corpus, sentences, synonyms and a tokenizer, and spread tokens over two vocabs."""
    # NOTE this feels very complicated
    sampler = draw(st.sampled_from([character_generator, word_generator]))
    vocab = draw(vocab_generator(sampler))
    other_vocab = draw(vocab_generator(sampler))
    assume(not (vocab == other_vocab)) # making sure the vocabs are not the same

    # making sure not to sample from tokens that appear in both vocabs
    tokens_in_vocab_not_in_other = sorted( set(vocab.keys()).difference(set(other_vocab.keys())) ) # sorting otherwise hypothesis gets mad
    tokens_in_other_not_in_vocab = sorted( set(other_vocab.keys()).difference(set(vocab.keys())) )
    assume(len(tokens_in_vocab_not_in_other) > 2 and len(tokens_in_other_not_in_vocab) > 2)

    # Determining how many tokens to draw
    draw_n_from_vocab = draw(st.integers(min_value=1, max_value=len(tokens_in_vocab_not_in_other)))
    draw_n_from_other_vocab = draw(st.integers(min_value=1, max_value=len(tokens_in_other_not_in_vocab)))

    # Drawing tokens from each vocab to add to the other vocab
    tokens_from_vocab = draw(st.lists(st.sampled_from(tokens_in_vocab_not_in_other), min_size=draw_n_from_vocab, max_size=draw_n_from_vocab, unique=True))
    tokens_from_other_vocab = draw(st.lists(st.sampled_from(tokens_in_other_not_in_vocab), min_size=draw_n_from_other_vocab, max_size=draw_n_from_other_vocab, unique=True))

    # Finding spots to put these synonyms into
    vocab_values, other_vocab_values = sorted(vocab.values()), sorted(other_vocab.values())
    add_to_vocab_at = draw(st.lists(st.sampled_from(vocab_values), min_size=draw_n_from_other_vocab, max_size=draw_n_from_other_vocab))
    add_to_other_vocab_at = draw(st.lists(st.sampled_from(other_vocab_values), min_size=draw_n_from_vocab, max_size=draw_n_from_vocab))

    # figuring out which keys are now synonyms
    _reverse_vocab = {value: key for (key, value) in vocab.items()}
    _reverse_other_vocab = {value: key for (key, value) in other_vocab.items()} 
    vocab_synonyms = [(_reverse_vocab[index], other_token) for (index, other_token) in zip(add_to_vocab_at, tokens_from_other_vocab)]
    other_vocab_synonyms = [(_reverse_other_vocab[index], token) for (index, token) in zip(add_to_other_vocab_at, tokens_from_vocab)]

    # updating the vocabs
    vocab.update({key: value for (key, value) in zip(tokens_from_other_vocab, add_to_vocab_at)})
    other_vocab.update({key: value for (key, value) in zip(tokens_from_vocab, add_to_other_vocab_at)})

    synonyms = vocab_synonyms + other_vocab_synonyms
    # creating corpus to check against
    corpus = set(vocab.keys()).union(set(other_vocab.keys()))
    return corpus, vocab, other_vocab, synonyms

@given(data=merge_vocabs_strategy())
def test_merge_vocabs(data):
    """Testing merge_vocabs() with standard vocab tests"""
    corpus, vocab, other_vocab, synonyms = data

    combined_vocab = merge_vocabs(vocab, other_vocab)
    _test_vocab(corpus, combined_vocab)
    for  synonym_list in synonyms:
        # checking if all the synonyms in the lists get the same value from the vocab
        assert len(set([combined_vocab[syn] for syn in synonym_list])) == 1
